        ...
        function animate() {
            ...
            score += 0.05;
            // La velocidad sigue aumentando sutilmente
            gameSpeed += 0.0003; 
            ...
            if (spawnTimer <= 0) {
                ...
                // Reducir el tiempo entre obstáculos dada la alta velocidad
                spawnTimer = (Math.random() * 40 + 40) - (gameSpeed * 1.5);
                if (spawnTimer < 35) spawnTimer = 35; // Mínimo de 35 para evitar obstáculos imposibles
            }
        ...
        function handleInput(e) {
            if (!gameRunning) {
                if ((e.code || e.type === 'mousedown' || e.type === 'touchstart') && !startScreen.classList.contains('hidden')) startGame();
                return;
            }
            if (e.type === 'keydown') {
                ...
                if (clientY && clientY > window.innerHeight / 2) dino.crouch(true);
                else dino.jump();
